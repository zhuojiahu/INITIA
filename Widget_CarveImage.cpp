...
// 	connect(pWidgetCarveInfo->ui.pushButton_balanceLine,SIGNAL(clicked()),this,SLOT(slots_setbalanceLine()));
...
void Widget_CarveImage::slots_save()
{
	...
	pMainFrm->Logfile.write(tr("Save carve info! "),OperationLog);
	...
	//初始化图像和链表
 	for (int i = 0; i < pMainFrm->m_sSystemInfo.iCamCount; i++)
 	{
		if(i==iCarvedCamNum)
		{
			...
			{
				disconnect(pMainFrm->pdetthread[pMainFrm->m_sCarvedCamInfo[i].m_iToNormalCamera],SIGNAL(signals_updateImage(QImage*, QString , QString ,QString , QString, QString, QList<QRect>,int )),imageShowItem,SLOT(slots_updateImage(QImage*, QString , QString ,QString , QString, QString, QList<QRect>,int )));
			}
		}
 	}	
...
void Widget_CarveImage::slots_setToCamera()
{
	...
	if(pMainFrm->m_sRealCamInfo[iCameraSN].m_iGrabType == 1)
	{
		if(pMainFrm->m_sRealCamInfo[iCameraSN].m_iTrigger == 1)
		{
			((CDHGrabberAVT*)pMainFrm->m_sRealCamInfo[iCameraSN].m_pGrabber)->AVTSetTriggerParam(AVTTriggerOn);
			pMainFrm->m_sRealCamInfo[iCameraSN].m_bGrabIsTrigger = true;
		}else if(pMainFrm->m_sRealCamInfo[iCameraSN].m_iTrigger == 0)
		{
			((CDHGrabberAVT*)pMainFrm->m_sRealCamInfo[iCameraSN].m_pGrabber)->AVTSetTriggerParam(AVTTriggerOff);
	...
	if(pMainFrm->m_sRealCamInfo[iCameraSN].m_iGrabType == 2)
	{
		//((CDHGrabberBasler*)pMainFrm->m_sRealCamInfo[iCameraSN].m_pGrabber)->BalserSetParam(Type_Basler_ExposureTimeAbs,iShuterTime);
	}		
	else if(pMainFrm->m_sRealCamInfo[iCameraSN].m_iGrabType == 3)
	{
		if(pMainFrm->m_sRealCamInfo[iCameraSN].m_iTrigger == 1)
		{
			((CDHGrabberBaumer*)pMainFrm->m_sRealCamInfo[iCameraSN].m_pGrabber)->BaumerSetTriggerParam(BaumerTriggerOn);
			pMainFrm->m_sRealCamInfo[iCameraSN].m_bGrabIsTrigger = true;
		}
		else if(pMainFrm->m_sRealCamInfo[iCameraSN].m_iTrigger == 0)
		{
			((CDHGrabberBaumer*)pMainFrm->m_sRealCamInfo[iCameraSN].m_pGrabber)->BaumerSetTriggerParam(BaumerTriggerOff);
			pMainFrm->m_sRealCamInfo[iCameraSN].m_bGrabIsTrigger = false;
		}
		((CDHGrabberBaumer*)pMainFrm->m_sRealCamInfo[iCameraSN].m_pGrabber)->BaumerSetParamPro(BaumerExposure,iShuterTime);
	}
	...
	if (pMainFrm->m_sSystemInfo.m_bIsIOCardOK)
	{
		...
		else if((2 == pMainFrm->m_sSystemInfo.m_iSystemType || 6 == pMainFrm->m_sSystemInfo.m_iSystemType) && iLastTriggerDelay != iTriggerDelay)//一体机
		{
			...
			case 5:
				...
				break;
			case 9:
				pMainFrm->m_vIOCard[1]->writeParam(59,iTriggerDelay);
				strPara = strPara.setNum(59,10);
				IOCardNumber = 1;
				break;
			case 6:
...
void Widget_CarveImage::slots_startTest()
{
	...
	{
	QMessageBox::information(this,tr("Infomation"),tr("Please Stop Detect First!"));
	return;
	}*/
	...
	else
	{
		...
		pMainFrm->m_cCombine.RemovAllResult();
		pMainFrm->m_cCombine.RemovAllError();
		pMainFrm->m_cCombine.m_MutexCombin.unlock();

		pMainFrm->m_cCombine1.m_MutexCombin.lock();
		pMainFrm->m_cCombine1.RemovAllResult();
		pMainFrm->m_cCombine1.RemovAllError();
	...
	if (pMainFrm->m_sSystemInfo.m_bIsIOCardOK)
	{
		if (0 == pMainFrm->m_sSystemInfo.m_iSystemType)
		{
			...
			case 5:
				...
				break;
			default:
				pMainFrm->Logfile.write(tr("NO Position%1").arg(iPosition),AbnormityLog);
				break;
			}
		}
	...
	if (pMainFrm->m_sSystemInfo.m_bIsIOCardOK)
	{
		...
		else if (2 == pMainFrm->m_sSystemInfo.m_iSystemType || 6 == pMainFrm->m_sSystemInfo.m_iSystemType)
		{
			...
			case 5:
				...
				break;
			case 9:
				iTriggerDelay = pMainFrm->m_vIOCard[1]->readParam(59);
				break;
			case 6:
				iTriggerDelay = pMainFrm->m_vIOCard[0]->readParam(57);
				break;
...
void Widget_CarveImage::slots_setbalanceLine()
{
