...
void Widget_Sample::initail()
{
	...
	checkBoxContinuousSampling = new QCheckBox;
	checkBoxContinuousSampling->setText(tr("Continuous Sampling"));
	...
	gridlayout->addWidget(checkBoxContinuousSampling,(pMainFrm->m_sErrorInfo.m_iErrorTypeCount-1)/4+1,0);
	...
	connect(lineEdit_sampleCount,SIGNAL(textChanged(QString)), this ,SLOT(changeSimpleCount()));
	connect(checkBoxContinuousSampling,SIGNAL(stateChanged(int)), this ,SLOT(changeSimpleCount()));
...
//  	}
 ...
 void Widget_Sample::changeSimpleCount()
 {
	 ...
	 sSampleInfo.bContinuousSampling = checkBoxContinuousSampling->isChecked();
