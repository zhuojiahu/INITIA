...
void WidgetTest::slots_StatisModeChanged(int index)
{
	...
	}else if(3 == iStatisMode)
	{
		...
		ui.label_2->setText(QString::fromLocal8Bit("报警最小值(%):"));
		ui.spinBox->setVisible(true);
		ui.spinBox_2->setVisible(true);
		ui.label_4->setVisible(true);
		ui.label_4->setText(QString::fromLocal8Bit("报警最大值(%):"));
		ui.label_13->setVisible(true);
		ui.label_13->setText(QString::fromLocal8Bit("换班报警间隔(m):"));
...
//	if (pMainFrm->m_sSystemInfo.m_bIsIOCardOK)
	...
	if (pMainFrm->m_sSystemInfo.iIsSample)
	{
		...
		{
			pMainFrm->m_vIOCard[0]->writeParam(76,m_nSampleDelay);//有后光电为中后，无后光电为中到剔废口
			pMainFrm->m_vIOCard[0]->writeParam(92,m_nSampleWidth);
			if (2 == pMainFrm->m_sSystemInfo.m_iSystemType||6 == pMainFrm->m_sSystemInfo.m_iSystemType||6 == pMainFrm->m_sSystemInfo.m_iSystemType)//一体机
			{
				pMainFrm->m_vIOCard[1]->writeParam(81,m_nSampleDelay);//有后光电后--剔废口
			}
			//pMainFrm->m_vIOCard[1]->writeParam(76,m_nSampleDelay);//有后光电为中后，无后光电为中到剔废口
		}
	}
...
void WidgetTest::slots_setToCard2()
{
	...
	if (pMainFrm->m_sSystemInfo.m_bIsIOCardOK)
	{
		...
		pMainFrm->m_vIOCard[1]->writeParam(76,m_nDistinct3to4);//2nd有后光电为中后，无后光电为中到剔废口
...
void WidgetTest::slots_updateIOcardCounter()
{
	if (pMainFrm->m_sSystemInfo.m_bIsIOCardOK)
	{
		...
		ui.label_23->setText(QString::fromLocal8Bit("接口卡补踢总数：")+QString::number(iCounter));
		ui.label_15->setText(QString::fromLocal8Bit("综合过检总数：")+QString::number(pMainFrm->m_sRunningInfo.nGSoap_ErrorTypeCount[0]));
		ui.label_24->setText(QString::fromLocal8Bit("综合过检踢废：")+QString::number(pMainFrm->m_sRunningInfo.nGSoap_ErrorCamCount[0]));
		ui.label_28->setText(QString::fromLocal8Bit("补踢过检总数：")+QString::number(pMainFrm->m_sRunningInfo.nGSoap_ErrorTypeCount[2]));
		ui.label_29->setText(QString::fromLocal8Bit("补踢踢废总数：")+QString::number(pMainFrm->m_sRunningInfo.nGSoap_ErrorCamCount[2]));
		...
		if (2 == pMainFrm->m_sSystemInfo.iIOCardCount)
		{
			...
			ui.label_25->setText(QString::fromLocal8Bit("接口卡补踢总数：")+QString::number(iCounter));
			ui.label_26->setText(QString::fromLocal8Bit("综合过检总数：")+QString::number(pMainFrm->m_sRunningInfo.nGSoap_ErrorTypeCount[1]));
			ui.label_27->setText(QString::fromLocal8Bit("综合过检踢废：")+QString::number(pMainFrm->m_sRunningInfo.nGSoap_ErrorCamCount[1]));
			ui.label_30->setText(QString::fromLocal8Bit("补踢过检总数：")+QString::number(pMainFrm->m_sRunningInfo.nGSoap_ErrorTypeCount[3]));
			ui.label_31->setText(QString::fromLocal8Bit("补踢踢废总数：")+QString::number(pMainFrm->m_sRunningInfo.nGSoap_ErrorCamCount[3]));
		}
	}
}
// 
